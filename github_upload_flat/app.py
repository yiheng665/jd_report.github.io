...
def main():
    ...
    c2.metric("平均价格", f"¥{filtered['price'].mean():,.0f}" if len(filtered) else "—")
    c3.metric("平均评论", f"{filtered['comments'].mean():,.0f}" if len(filtered) else "—")
    c4.metric("平均好评率", f"{filtered['rating'].mean():.1f}%" if len(filtered) else "—")
    ...
    with tab1:
        if filtered.empty:
            ...
            with right:
                top = filtered.nlargest(20, "comments")
                fig = px.scatter(top, x="price", y="comments", color="category", hover_name="title", size="rating", title="热门商品价格—评论分布")
                st.plotly_chart(fig, width="stretch")
